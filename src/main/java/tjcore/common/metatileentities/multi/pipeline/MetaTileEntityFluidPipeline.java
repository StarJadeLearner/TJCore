...
public class MetaTileEntityFluidPipeline extends MultiblockWithDisplayBase implements IPipeline {
    ...
    FluidTankList inputs;
    FluidTankList outputs;
    int pipelikeTotal;
    int pipelikeRequirement = -1;
    NotifiableItemStackHandler pipelikeInput = new NotifiableItemStackHandler(1, this, false);
    MetaTileEntityFluidPipeline connection;

    public MetaTileEntityFluidPipeline(ResourceLocation metaTileEntityId) {
    ...
    public void setConnection(IPipeline pipeline) {
        ...
        if (pipeline.getType() == getType()) {
            this.connection = (MetaTileEntityFluidPipeline) pipeline;
            isConnected = true;
    ...
    public void disconnect(boolean recyclePipes) {
        ...
        if (recyclePipes) {
            pipelikeInput.insertItem(0, OreDictUnifier.get(OrePrefix.pipeNormalFluid, Steel, Math.min(Math.min(pipelikeTotal, pipelikeRequirement), 64)), false);
        }
    ...
    protected void formStructure(PatternMatchContext context) {
        ...
        inputs = new FluidTankList(true, getAbilities(IMPORT_FLUIDS));
        outputs = new FluidTankList(true, getAbilities(EXPORT_FLUIDS));
    }
    ...
    protected void updateFormedValid() {
        if (getOffsetTimer() % 20 == 0 && this.isSource && connectionValid && connection != null) {
            GTTransferUtils.transferFluids(inputs, connection.outputs);
        }
        if (!connectionValid && connection != null) {
            if (getOffsetTimer() % 5 == 0 && !pipelikeInput.getStackInSlot(0).isEmpty() && pipelikeRequirement > 0) {
                ...
                if (stack.isItemEqual(OreDictUnifier.get(OrePrefix.pipeNormalFluid, Steel))) {
    ...
    public MetaTileEntity createMetaTileEntity(IGregTechTileEntity iGregTechTileEntity) {
        return new MetaTileEntityFluidPipeline(metaTileEntityId);
    }
